...
131. 泛型方法的类型变量放在修饰符的后面，返回类型的前面。
132. 泛型方法可以定义在普通类里，也可以定义在泛型类里。
133. 当调用一个泛型方法时，在方法名前的尖括号中放入具体的类型。
134. 大多数情况下，编译器可以有足够的信息能够推断出具体的类型，可以省略类型参数。
135. 可以通过对类型变量T设置_限定(bound)_来实现对类型变量的约束;限定关键字是```extends```;限定类型用```&```来分隔，而逗号用来分隔类型变量；如果用一个类作为限定，它必须是限定列表的第一个。

	```java
	<T extends Comparable & Serializable>
	```

136. 无论何时定义一个泛型类型，都自动提供了一个相应的__原始类型(raw type)__；原始类型的名字就是删去类型参数后的泛型类型名。_擦除_类型变量，并替换为限定类型（无限定的变量用Object代替）;因此，java的泛型与C++的不同，它没有__模板代码膨胀__;
137. 为了提高效率，应该讲标签(tagging)接口（没有方法的接口）放在边界列表的末尾。
138. 当程序调用泛型方法的时候，如果擦除返回类型，编译器插入强制类型转换。
139. 需要记住有关java泛型转换的事实：

		a. 虚拟机中没有泛型。
		b. 所有的类型参数都用它们的限定类型替换。
		c. 桥方法被合成来保持多态。(在虚拟机中用参数和返回类型来确认一个方法，当然代码中的返回类型不是方法的标签之一)
		d. 为保持类型安全性，必要时插入强制类型转换。
		
140. java泛型的约束与局限性。

		a. 不能用基本类型实例化类型参数。(原因是类型擦除)
		b. 运行时类型查询只适用于原始类型。无论何时使用__instanceof__或涉及泛型类型的强制类型转换表达式都会看到一个编译器警告；同样，getClass方法总是返回原始类型。
		c. 不能创建参数化类型的数组。因为擦除会使数组对其它元素类型的检查判断失效。
		d. 不能实例化类型变量。
